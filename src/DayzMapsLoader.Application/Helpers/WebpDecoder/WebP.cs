...
internal sealed class WebP : IDisposable
{
    ...
    /// <summary>
    /// Compute PSNR, SSIM or LSIM distortion metric between two pictures. Warning: this function is
    ...
    private byte[] AdvancedEncode(Bitmap bmp, WebPConfig config, bool info)
    {
        ...
        try
        {
            ...
            UnsafeNativeMethods._onCallback = new UnsafeNativeMethods.WebPMemoryWrite(MyWriter);
            wpic.writer = Marshal.GetFunctionPointerForDelegate(UnsafeNativeMethods._onCallback);
            ...
            //Remove OnCallback
            UnsafeNativeMethods._onCallback = null;
